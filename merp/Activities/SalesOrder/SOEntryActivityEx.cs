...
{
	[Activity (Label = "SALES ORDER",ConfigurationChanges=Android.Content.PM.ConfigChanges.Orientation | Android.Content.PM.ConfigChanges.ScreenSize)]			
	public class SOEntryActivityEx : Activity,IEventListener,Android.Views.View.IOnKeyListener
	{
	...
	//	string TRXTYPE;
		...
		void EditItem(string cnno, int id)
		{
			using (var db = new SQLite.SQLiteConnection(pathToDatabase))
			{
				var list = db.Table<SaleOrderDtls>().Where(x=>x.sono==cnno&& x.ID==id).ToList<SaleOrderDtls>();
				if (list.Count > 0) {
		...
		void DeleteItem(SaleOrderDtls invdtls)
		{
			using (var db = new SQLite.SQLiteConnection(pathToDatabase))
			{
				var list = db.Table<SaleOrderDtls>().Where(x=>x.sono==invdtls.sono&& x.ID==invdtls.ID).ToList<SaleOrderDtls>();
				if (list.Count > 0) {
					db.Delete (list [0]);
					var arrlist= listData.Where(x=>x.sono==invdtls.sono&& x.ID==invdtls.ID).ToList<SaleOrderDtls>();
					if (arrlist.Count > 0) {
...
//		}
		...
		void CancelReceipt()
		{
			string sono = Intent.GetStringExtra ("invoiceno") ?? "AUTO";
			using (var db = new SQLite.SQLiteConnection (pathToDatabase)) {
				var itemlist = db.Table<SaleOrderDtls> ().Where (x => x.sono == sono);	
				double ttlamt= itemlist.Sum (x => x.netamount);
				double ttltax= itemlist.Sum (x => x.tax);
				var invlist =db.Table<SaleOrder> ().Where (x => x.sono == sono).ToList<SaleOrder> ();
				if (invlist.Count > 0) {
		...
		void updatePrintedStatus(SaleOrder inv)
		{
			using (var db = new SQLite.SQLiteConnection (pathToDatabase)) {
				...
				if (list.Count > 0) {
					...
					var list2 = db.Table<SaleOrderDtls> ().Where (x => x.sono == inv.sono).ToList<SaleOrderDtls> ();
					if (list2.Count > 0) {
		...
		private void DeleteInv()
		{
			try {
				using (var db = new SQLite.SQLiteConnection (pathToDatabase)) {
					var list = db.Table<SaleOrderDtls> ().Where (x => x.sono == cnno).ToList<SaleOrderDtls> ();
					var list2 = db.Table<SaleOrder> ().Where (x => x.sono == cnno).ToList<SaleOrder> ();
					if (list2.Count > 0) {
						string trxtype = "CS";
						AdNumDate adNum = DataHelper.GetNumDate (pathToDatabase, list2 [0].sodate, trxtype);
						if (cnno.Length > 5) {
